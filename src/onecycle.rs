...
impl OneCycleLR {
    ...
    fn interpolate(&self, start: f64, end: f64, progress: f64) -> f64 {
        match self.anneal_strategy {
            AnnealStrategy::Linear => start + (end - start) * progress,
            AnnealStrategy::Cos => {
...
impl Scheduler for OneCycleLR {
    fn step(&mut self, _loss: f64) {
        ...
        if self.step_count >= self.total_steps {
            ...
            self.lr = self.calculate_lr(phase, progress);
        }
    }
...
mod tests {
    ...
    fn three_phase_linear() {
        let mut scheduler =
            OneCycleLR::new(0.2, 12, 0.25, AnnealStrategy::Linear, 20.0, 1000.0, true);
    ...
    fn phase_boundaries() {
        let mut scheduler =
            OneCycleLR::new(1.0, 10, 0.4, AnnealStrategy::Linear, 10.0, 100.0, false);
        ...
        let max_warmup_lr = phase_lrs[0..=4]
            .iter()
            .map(|(_, lr)| *lr)
            .fold(0.0f64, f64::max);
    ...
    fn cosine_vs_linear_annealing() {
        let mut cos_scheduler =
            OneCycleLR::new(1.0, 8, 0.5, AnnealStrategy::Cos, 10.0, 100.0, false);
        let mut lin_scheduler =
            OneCycleLR::new(1.0, 8, 0.5, AnnealStrategy::Linear, 10.0, 100.0, false);
    ...
    fn beyond_total_steps() {
        let mut scheduler =
            OneCycleLR::new(1.0, 5, 0.4, AnnealStrategy::Linear, 10.0, 100.0, false);
